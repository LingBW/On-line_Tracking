...
try:
    if not stt:
        ...
        stime = parse(stt,dayfirst=True) #datetime(2015,2,10,12,0,0,0,pytz.UTC     
        start_time = pytz.utc.localize(stime)+timedelta(hours=ditnu);#print start_time
except:
...
if MODEL in GRID:
    ...
    try:
        ...
        if MODEL == '30yr':
            ...
            print '<h2>Track time: Error! Model(%s) only works the past and future 3 days(%s - %s). <br>Tips: check step 5 or 6.</h2>'%(MODEL,mstt,mett)
        print '</head></html>'
        sys.exit()
...
if MODEL=='ROMS': 
    ...
    #for i in range(stp_num):
    point = get_obj.get_track(st_lon,st_lat,lon_rho,lat_rho,u,v,track_way)
    if len(point['lon'])==1: 
        print '<script type="text/javascript">window.alert("Start point is out of the Model(%s) Zone. Tips: change the Model(step 2), try again.")</script>'%MODEL
...
<div id="footer" ><a href="http://comet.nefsc.noaa.gov/ioos/track/index1.html"><input type="button" value="Track again" /></a></div>
"""%(MODEL,track_way,tds)
